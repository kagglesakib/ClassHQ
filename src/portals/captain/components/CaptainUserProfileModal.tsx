...
export const CaptainUserProfileModal: React.FC<CaptainUserProfileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 overflow-hidden bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-4xl h-[94vh] sm:h-auto sm:max-h-[88vh] bg-white border border-sky-200 rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Android Native Grab Handle */}
        <div className="sm:hidden pt-2.5 pb-1 flex justify-center bg-sky-50/50 shrink-0">
          <div className="w-12 h-1 bg-sky-300/80 rounded-full" />
        </div>

        {/* Modal Top Bar */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-2.5 sm:py-3.5 border-b border-sky-100 bg-sky-50/50 shrink-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest bg-sky-100 text-sky-800 border border-sky-200 shrink-0">
              Institutional Profile Dossier
            </span>
            <span className="font-mono text-xs font-black text-sky-600 truncate">
              ID: {currentUser.id}
            </span>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-sky-100/60 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 bg-sky-50/30 border-b border-sky-100 shrink-0">
            ...
            <div className="flex items-start sm:items-center gap-3.5 sm:gap-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                  ...
                  {currentUser.role === 'admin' && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-sky-100 text-sky-800 border border-sky-200 inline-flex items-center gap-1">
                ...
                <p className="text-xs font-bold text-slate-600 mt-1">
                  {currentUser.rollNumber && (
                    <>
                      Roll No: <span className="font-mono text-sky-700 font-black text-sm">{currentUser.rollNumber}</span> •{' '}
                    </>
                  )}
              ...
              {currentUser.approval === 'pending' && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-3 py-1.5 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1 shadow-md"
                  >
        ...
        {/* Tab Navigation */}
        <div className="grid grid-cols-3 gap-1 px-2 sm:px-6 pt-2 border-b border-sky-200/80 bg-sky-50/50 shrink-0">
          <button
            type="button"
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-5 sm:space-y-6 flex-1 min-h-0 text-slate-800">
          {loading ? (
            <div className="py-16 text-center text-xs font-bold text-sky-600 animate-pulse flex flex-col items-center justify-center gap-2">
            ...
            <>
              ...
              {activeTab === 'profile' && (
                <div className="space-y-6">
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase tracking-widest text-sky-700 mb-3">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-200/80 flex items-start gap-3">
                        <Mail className="w-4 h-4 text-sky-600 mt-1 shrink-0" />
                        <div>
                      ...
                      <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-200/80 flex items-start gap-3">
                        <Phone className="w-4 h-4 text-sky-600 mt-1 shrink-0" />
                        <div>
                      ...
                      <div className="sm:col-span-2 p-4 rounded-2xl bg-sky-50/50 border border-sky-200/80 flex items-start gap-3">
                      ...
                      <div className="sm:col-span-2 p-4 rounded-2xl bg-sky-50/50 border border-sky-200/80 flex items-start gap-3">
                        <Calendar className="w-4 h-4 text-sky-600 mt-1 shrink-0" />
                        <div>
              ...
              {activeTab === 'attendance' && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-200/80 space-y-1">
                      <span className="text-[10px] font-black uppercase tracking-widest text-sky-700 block">Absent</span>
                      <span className="text-2xl font-black text-sky-600">{attendanceStats?.daysAbsent ?? 0}</span>
                    </div>
                  </div>

                  {/* Attendance Log Table */}
                  <div className="p-4 rounded-2xl bg-white border border-sky-200/80 space-y-3 shadow-2xs">
                    <h4 className="text-xs font-black uppercase tracking-widest text-sky-700">
                      ...
                      <div className="space-y-2.5">
                        {attendanceRecords.map((rec, index) => (
                          <div
                            ...
                            className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 hover:border-sky-200 transition-all space-y-1.5"
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase inline-flex items-center gap-1 ${
                                ...
                                {rec.status === 'Absent' && <XCircle className="w-3 h-3 text-sky-600 shrink-0" />}
                  ...
                  {leaveRequests.length > 0 ? (
                    <div className="space-y-3">
                      {leaveRequests.map((lv, index) => (
                        <div key={lv.id ? `lv-${lv.id}-${index}` : `lv-idx-${index}`} className="p-4 rounded-2xl bg-white border border-sky-200/80 space-y-2 shadow-2xs">
                          ...
                          {lv.reviewedBy && (
                            <div className="pt-2 border-t border-sky-100 text-[11px] text-slate-500 flex items-center justify-between">
                    ...
                    <div className="py-12 text-center text-xs font-bold text-slate-500 p-6 rounded-2xl bg-sky-50/50 border border-sky-200/60">
        ...
        {/* Modal Footer */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-sky-100 bg-sky-50/50 flex items-center justify-between shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-xs"
          >
            Close Dossier
          </button>
        </div>
